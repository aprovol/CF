...
def mix(samples: list, proportions: list, totalcoverage: int, seed: int):
    ...
    assert np.isclose(np.sum(proportions), 1.0), 'Provided mixture proportions do not add to 1'
...
# Generate N random mixtures of proportions for any number of tissues
def generate_random_fractions(ntissues, nfractions):
    fractions = []
    for _ in range(nfractions):
        p = dirichlet(np.ones(ntissues))  # uniform Dirichlet distribution
        fractions.append(p)
    return np.array(fractions)

# Number of mixtures you want (was 11 before)
nfractions = 10
...
df = pd.DataFrame(fakeClusters[:counter], index=mixedSampleNames, columns=geneNames)
